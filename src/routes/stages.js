...
// Body: { completed: boolean, date?: 'YYYY-MM-DD', note?: string, owner_id?: number, updated_by?: string, not_applicable?: boolean }
...
router.patch('/:stageKey', async (req, res, next) => {
  try {
    ...
    const { completed, date, note, owner_id, updated_by, not_applicable } = req.body || {};
    ...
    const result = await pool.query(
      `INSERT INTO product_stages (product_id, stage_key, completed_at, note, owner_id, updated_by, not_applicable)
       ...
       ON CONFLICT (product_id, stage_key) DO UPDATE SET
         ...
         not_applicable = EXCLUDED.not_applicable, updated_at = now()
       RETURNING *`,
      [id, stageKey, completedAt, note || null, owner_id || null, updated_by || null, notApplicableValue]
    );
